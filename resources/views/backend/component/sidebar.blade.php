...
<ul class="navbar-nav bg-gradient-danger sidebar sidebar-dark accordion" id="accordionSidebar">
    ...
    <a class="sidebar-brand d-flex align-items-left justify-content-left" href="#">
        <div class="sidebar-brand-text">{{ App\Setting::where('slug', 'nama-toko')->get()->first()->description }}
        </div>
    </a>
    ...
    <li class="nav-item">
        <a class="nav-link {{ active(['car.index', 'manufacture.index'], 'collapsed') }}" href="#"
            data-toggle="collapse" data-target="#car" aria-expanded="true" aria-controls="car">
            <i class="fas fa-fw fa-car"></i>
            <span>Manajemen Data</span>
        </a>
        <div id="car" class="collapse {{ active(['car.index', 'manufacture.index'], 'show') }}"
            aria-labelledby="headingTwo" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
                <a class="collapse-item {{ active('car.index') }}" href="{{ route('car.index') }}">Data Mobil</a>
                <a class="collapse-item {{ active('manufacture.index') }}"
                    href="{{ route('manufacture.index') }}">Data
                    Merek</a>
            </div>
        </div>
    </li>
    ...
    <li class="nav-item">
        <a class="nav-link {{ active('transaction.*', 'collapsed') }}" href="#" data-toggle="collapse"
            data-target="#transaksi" aria-expanded="true" aria-controls="transaksi">
            <i class="fas fa-fw fa-book"></i>
            <span>Sewa</span>
        </a>
        <div id="transaksi" class="collapse {{ active('transaction.*', 'show') }}" aria-labelledby="headingTwo"
            data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
                <a class="collapse-item {{ active('transaction.create') }}"
                    href="{{ route('transaction.create') }}">Sewa</a>
                <a class="collapse-item {{ active('transaction.index') }}"
                    href="{{ route('transaction.index') }}">List Sewa</a>
                <a class="collapse-item {{ active('transaction.history') }}"
                    href="{{ route('transaction.history') }}">Riwayat Sewa</a>
            </div>
        </div>
    </li>
